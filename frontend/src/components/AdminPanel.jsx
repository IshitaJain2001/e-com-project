...
dispatch({
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch("https://e-com-project-1-db0p.onrender.com/products/add-product", {
        method: "POST",
        body: formData,
      });
  ...
  const handleUpdate = async (id, updatedProduct) => {
    try {
      ...
      if (newImages.length > 0) {
        ...
        res = await fetch(`https://e-com-project-1-db0p.onrender.com/products/update-product/${id}`, {
        method: "PATCH",
        body: formData,
        });
        } else {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`https://e-com-project-1-db0p.onrender.com/products/delete-product/${id}`, {
        method: "DELETE",
      });
